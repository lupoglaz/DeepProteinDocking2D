...
class RMSDIndicator(nn.Module):
    ...
    def forward(self, ligand, translation, rotation, coords):
        ...
        rmsd = rmsd + 2.0 * torch.sum(
            torch.sum(T.unsqueeze(dim=-1) * ((R2 - R1).unsqueeze(dim=2).unsqueeze(dim=3)), dim=4) * C, dim=-1)
        return rmsd
...
class EQInteraction(nn.Module):
    ...
    def forward(self, scores, angles, ligand=None):
        ...
        P = torch.exp(-scores)
        coords = torch.sum(self.coords * P.unsqueeze(dim=1), dim=(2, 3, 4)) / (
            torch.sum(P, dim=(1, 2, 3)).unsqueeze(dim=1))
...
#             nn.Conv3d(1, 4, kernel_size=self.kernel, padding=self.pad, stride=self.stride, dilation=self.dilation,
#                       bias=False),
#             nn.ReLU(),
#             nn.Conv3d(4, 1, kernel_size=self.kernel, padding=self.pad, stride=self.stride, dilation=self.dilation,
#                       bias=False),
#             nn.ReLU(),
#         )
#
