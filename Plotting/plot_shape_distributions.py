...
def get_dict_counts(shape_params):
    alpha_counts = []
    numpoints_counts = []
    params_list = []
...
def get_unique_fracs(counts):
    unique, counts = get_counts(counts)
...
def check_missing_examples(combination_list, found_list, protein_shapes, params_list):
    missing_list = []
    missing_indices = []
...
def get_shape_distributions(data, alpha_counts, numpoints_counts, params_list, debug=False):
    protein_shapes = data.proteins
    shape_params = data.params

    alphas_unique, alphas_fracs, alphas_barwidth = get_unique_fracs(alpha_counts)
    numpoints_unique, numpoints_fracs, numpoints_barwidth = get_unique_fracs(numpoints_counts)
    ...
    if len(found_list) < len(combination_list):
        ...
        indices = check_missing_examples(combination_list, found_list, protein_shapes, params_list)
...
def plot_shape_and_params(protein_pool, dataset_name):
    ...
    plt.savefig(dataset_name + str(len(data.proteins)) + 'pool_combined_shapes_params')
    plt.show()
